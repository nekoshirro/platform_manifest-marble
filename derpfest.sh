...
start_build_process() {
    ...
    *Start Time:* $(date '+%Y-%m-%d %H:%M:%S %Z')"
    ...
    # Device Trees
    git clone https://github.com/nekoshirro/platform_device_xiaomi_marble.git device/xiaomi/marble -b derpfest-16 --depth 1
    ...
    # Sign with Derpfest keys
    git clone https://github.com/nekoshirro/android_vendor_priv-keys.git -b derpfest vendor/lineage-priv/keys
    ...
    echo "========================="
    mka derp -j$(nproc --all) 2>&1 | tee log.txt
    ...
    *Status:* $status_text"
    ...
    if [[ $BUILD_STATUS -eq 0 ]]; then
        ...
        chmod +x go-up
        ./go-up out/target/product/marble/Derp*marble*.zip
    else
