...
start_build_process() {
    ...
    *Start Time:* $(date '+%Y-%m-%d %H:%M:%S %Z')"
    ...
    # Device Trees
    git clone https://github.com/nekoshirro/platform_device_xiaomi_marble.git device/xiaomi/marble -b infinity-16 --depth 1
    ...
    # Sign build with custom signing keys from Project Infinity-X
    git clone https://github.com/ProjectInfinity-X/vendor_infinity-priv_keys-template.git vendor/infinity-priv/keys --depth 1
    chmod +x vendor/infinity-priv/keys/keys.sh
    pushd vendor/infinity-priv/keys
    ./keys.sh
    popd
    ...
    *Status:* $status_text"
    ...
    if [[ $BUILD_STATUS -eq 0 ]]; then
        ...
        chmod +x go-up
        ./go-up out/target/product/marble/Project*marble*.zip
    else
