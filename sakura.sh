...
start_build_process() {
    ...
    *Start Time:* $(date '+%Y-%m-%d %H:%M:%S %Z')"
    ...
    # Device Trees
    git clone https://github.com/nekoshirro/platform_device_xiaomi_marble.git device/xiaomi/marble -b sakura-16 --depth 1
    ...
    *Status:* $status_text"
    ...
    if [[ $BUILD_STATUS -eq 0 ]]; then
        ...
        chmod +x go-up
        ./go-up out/target/product/marble/*Sakura*marble*.zip
    else
