...
start_build_process() {
    ...
    *Start Time:* $(date '+%Y-%m-%d %H:%M:%S %Z')"
    ...
    # Device Trees
    git clone https://github.com/nekoshirro/platform_device_xiaomi_marble.git device/xiaomi/marble -b shinkai-16 --depth 1
    ...
    git clone --recurse-submodules https://github.com/nekoshirro/platform_kernel_xiaomi_marble.git -b backup kernel/xiaomi/marble --depth 1
    ...
    lunch shinkai_marble-bp4a-user
    ...
    *Status:* $status_text"
    ...
    if [[ $BUILD_STATUS -eq 0 ]]; then
        ...
        chmod +x go-up
        ./go-up out/target/product/marble/*marble*.zip
    else
